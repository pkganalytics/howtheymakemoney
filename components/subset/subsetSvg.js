...
// 		  .enter().append()
// 		  .call((d) => console.log('node2 after enter append=', svg.selectAll(".node").data(graph2.nodes, d => d.name).enter().append()))
// 		  .exit()
// 		  .call((d) => console.log('node2 after exit remove=', svg.selectAll(".node").data(graph2.nodes, d => d.name).enter().exit()))

const rect2 = svg.selectAll('.node rect')
	  .data(graph2.nodes, d => d.name)
		  .call((d) => console.log('enter rect2 after adding data=', svg.selectAll(".node").data(graph2.nodes, d => d.name)))
 		  .call((d) => console.log('node2 rect2 enter append=', svg.selectAll(".node").data(graph2.nodes, d => d.name).enter().append()))
 		  .call((d) => console.log('node2 after exit remove=', svg.selectAll(".node").data(graph2.nodes, d => d.name).enter().exit()))
	  .transition()
	  .duration(3000)
	  .attr("x", d => d.x0)
